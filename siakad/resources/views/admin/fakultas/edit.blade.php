<x-layout>
    ...
    <form class="flex h-screen"action="/admin/kelola-fakultas/{{$fakultas->kode_fakultas}}" method="POST">
    @csrf
    @method('PATCH')
    <fieldset class="fieldset bg-base-200 border-base-300 rounded-box w-xs h-80 border p-4 mx-auto">
        ...
        <input type="text" class="input" name="kode_fakultas" placeholder="" value="{{$fakultas->kode_fakultas}}"/>
        <x-forms.error name='kode_fakultas'/>

        <label class="label font-bold" for="nama_fakultas" >nama_fakultas</label>
        <input type="text" class="input" name="nama_fakultas" placeholder="" value="{{$fakultas->nama_fakultas}}" />
